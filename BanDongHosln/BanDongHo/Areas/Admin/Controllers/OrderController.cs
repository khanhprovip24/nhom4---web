...
namespace BanDongHo.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult ExportToTxt()
        {
            ...
            var filePath = Server.MapPath("~/Orders.txt");
            using (var writer = new StreamWriter(filePath))
            {
                ...
                {
                    writer.WriteLine($"MÃ khách hàng: {order.MAKH}");
                    writer.WriteLine($"Tên Khách hàng: {order.KHACHHANG.TENKH}");
                    writer.WriteLine($"Ngày đặt hàng: {order.NGAYDAT}");
                    writer.WriteLine($"SDT: {order.SDT}");
                    writer.WriteLine($"Địa chỉ: {order.DIACHIGIAO}");
                    writer.WriteLine($"Mô tả : {order.MOTA}");
                    writer.WriteLine($"Tổng tiền: {order.TONGTIEN}");

                    writer.WriteLine();
                }
            }

            return File(filePath, "text/plain", "Orders.txt");
        }
    }
}